  ...
  suite('Routing tests', function() {
      ...
      test('Test POST /api/books with title', function(done) {
        chai.request(server)
          ...
          .end((err, res) => {
            ...
            assert.equal(res.body.title, "Test Book", "Title should match input");
            done()
          });
      });
    ...
    suite('POST /api/books/[id] => add comment/expect book object with id', function(){
          ...
          .end((err, res) => {
            ...
            chai.request(server)
              ...
              .end((err, res) => {
                ...
                assert.property(res.body, "comments", "Response should have comments")
                assert.isArray(res.body.comments, "Comments should be an array");
                assert.include(res.body.comments, "Great book","Comments should include the comment");
                done();
              });
          });
      });
      ...
      test('Test POST /api/books/[id] with comment, id not in db', function(done){
        chai.request(server)
          .post("/api/books/invalididwithcomment")
    ...
    suite('DELETE /api/books/[id] => delete book object id', function() {
      ...
      test('Test DELETE /api/books/[id] with  id not in db', function(done){
        chai.request(server)
          .delete("/api/books/invalididdelete")
          .end((err, res) => {
            assert.equal(res.status, 200);
            assert.equal(res.text, "no book exists");
            done();
